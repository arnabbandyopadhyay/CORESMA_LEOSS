...
dat<-read.table('/home/arnab/Downloads/Leoss_data/LEOSS_Bandyopadhyay_2022-05-27/SUF_Bandyopadhyay_220527.csv', sep = ';')
col<-dat[1,]
...
Ddimer_nan<-c(7,999,1000,'Missing')
...
emp_data_uc<-matrix('NA', nrow=12179, ncol = length(imp_lab_params)+1)
to_keep<-NULL
j<-1
count<-1
count2<-0
for (i in imp_lab_params){
  ...
  # #  clean_uc<-clean_uc[-c(which(clean_uc[,2] %in% remove[[j]])),]
  clean_uc<-clean_uc[-c(which(clean_uc[,2] %in% c('*',get(paste0(lab_params[i],'_nan'))))),c(2,3)]
  colnames(clean_uc)[1]<-c(lab_params[i])
  clean_uc$Cond<-rep('UC', nrow(clean_uc))
  print(paste('length uc', dim(clean_uc)[1]))
  # 
  ...
  if(row>0){
    ...
    if (!emp_data_uc[1,1]==clean_uc$Cond[1]){
      ...
      # emp_data_cr[1:row,count]<-as.numeric(as.character(clean_cr[,1]))
      emp_data_uc[1:row,(count2+1)]<-clean_uc[,1]
    }
  }
}
emp_data_uc<-data.frame(emp_data_uc)
colnames(emp_data_uc)<-c('Cond',lab_params[to_keep])

emp_data_uc<-emp_data_uc[1:5000,na.omit(colnames(emp_data_uc))]
...
emp_data<-rbind(emp_data_co[1:1200,colnames(emp_data_uc)], emp_data_cr[1:1000,colnames(emp_data_uc)], emp_data_uc[1:1300,])
for (i in 2:17){
  emp_data[,i]<-factor(emp_data[,i],levels=1:12)
}
emp_data[,1]<-factor(emp_data[,1],levels=c('CO','CR','UC'))
#emp_data<-emp_data[,-c(3,4,9)]
plot_na_pareto(emp_data, only_na = T)
...
Model_svm <- train(Cond ~ ., data = TrainingSet,
               ...
               tuneGrid = data.frame(degree=1,scale=1,C=1),metric = "ROC"
)
...
resamps <- resamples(list(RF=Model_rf, XGB = Model_xgb))
summary(resamps) 
...
Model_xgb <- train(Cond ~ .,data=TrainingSet,method='xgbTree',trControl=ctrl)
...
#pdf("classification_uc_co_cr.pdf", width=10)
ggplot(data=model_dat, aes(x=variable, y=Overall, fill=model)) +
  geom_bar(stat="identity", position=position_dodge())+theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+ggtitle('Importance of variable in classifying UC, CO, CR')
